...
async fn trace_state_diff(
    ...
    let state_diff = provider
        ...
        .await?
        .state_diff
        .ok_or(anyhow!("state diff does not exists"))?
        .0;
...
pub async fn mempool_watching(target_address:String) -> Result<()> {
    ...
    //step #1:Using cfmms-rs to sync all pools create on Uniswap V3
    ...
    let dexes: Vec<_> = dexes_data.into_iter()
    ...
    // I've put all the logic into mempool_watching so that it's easier to 
    // flollow along
    {
